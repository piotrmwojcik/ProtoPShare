        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                plt.imsave(fname=os.path.join(dir_for_saving_images,
                           ...
                           vmax=1.0)
                #import torchvision
                original_mask = patch.original_mask
                bw_array = (original_mask.astype(np.uint8) * 255)
                from PIL import Image
                bw_image = Image.fromarray(bw_array.squeeze(0), mode='L')
                #to_pil = torchvision.transforms.transforms.ToPILImage()
                #msk = to_pil(patch.original_mask)
                bw_image.save(os.path.join(dir_for_saving_images,
                ...
                high_act_bw_array = (original_mask.squeeze(0).astype(np.uint8) * 255)[high_act_patch_indices[0]:high_act_patch_indices[1],
                                                    high_act_patch_indices[2]:high_act_patch_indices[3]]
                from PIL import Image
                high_act_bw_image = Image.fromarray(high_act_bw_array, mode='L')

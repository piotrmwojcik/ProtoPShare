...
def join_prototypes(ppnet, p, joint_optimizer=None, warm_optimizer=None, last_layer_optimizer=None):
    """
    ...
    prototypes = ppnet.module.prototype_vectors.data.squeeze()
    distances = calculate_distances(prototypes)
    k = torch.kthvalue(distances.min(0)[0].cpu(), int(p * distances.cpu().size(0)))[0].item()
    dist_iterator = 0
    no_of_prototypes = len(distances)
    proto_joined = []
    print(f"distances smaller than {k:.4g}: {(distances.detach().cpu().numpy() <= k).sum()}")
    ...
    while dist_iterator < no_of_prototypes:
        ...
        if (proto_distanses <= k).any():
            ...
            proto_joined.append([dist_iterator, to_join])
            with torch.no_grad():
...
def join_prototypes_by_activations(ppnet, p, test_loader, joint_optimizer=None, warm_optimizer=None, last_layer_optimizer=None, no_p=None):
    """
    Join similar prototypes
    :param ppnet: model
    :param p: percentage of closest prototypes which should be joined
    :param test_loader loader for data
    """

    for idx, (search_batch_input, search_y) in enumerate(test_loader):
#        print('batch {}'.format(idx))
        ...
        with torch.no_grad():
            ...
            _, activation = ppnet.forward(search_batch)
        if idx == 0:
            activations = activation
        else:
            activations = torch.cat((activations, activation))

    distances_act = calculate_distances(torch.transpose(activations, 1, 0))  # torch.norm(activations.transpose(1, 0).cpu()[:, None] - activations.transpose(1, 0).cpu(), dim=2,
    #                           p=2)
    assert 0 < p < 1
    ...
    if no_p is None:
        ...
        print(distances_act.shape)
        print(torch.kthvalue(distances_act.min(0)[0].cpu(), no_p).values)

        k = torch.kthvalue(distances_act.min(0)[0].cpu(), no_p).values[0].item()
    dist_iterator = 0
